...
reviews.push({name: "Karen", rating: 0, feedback: "The manager was rude"});
console.log(reviews);
...
*/
 ...
 function getReviewByRating(reviews, stars) {
  ...
  for(let review of reviews)
    if(stars <= review.rating && review.rating < stars+1)
      reviewStars.push(review);
...
function getLongReviews(reviews) {
  let longReviews = [];
  for(let review of reviews)
    if(review.feedback.split(' ').length > 15)
